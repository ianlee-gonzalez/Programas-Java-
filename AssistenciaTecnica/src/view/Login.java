...
public class Login extends JFrame {
	...
	private void status() {
		...
		try {
			...
			// mudando o icone do rodapï¿½ quando conectar no banco de dados
			if (con == null) {
			...
			// IMPORTANTE sempre encerrar a conexï¿½o
			con.close();
		} catch (Exception e) {
	...
	private void logar() {
		if (txtLogin.getText().isEmpty()) {
			...
			try {
				...
				// a linha abaixo executa a query(instruï¿½ï¿½o SQL) armazenando o resultado no
				// objeto rs
				ResultSet rs = pst.executeQuery();
				// se existir o login e senha correspondente
				if (rs.next()) {
					// capturar o perfil do usuï¿½rio
